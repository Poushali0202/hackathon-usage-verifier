...
class Target:
    ...
    def from_preset(cls, cfg: dict) -> "Target":
        ...
        return cls(
            slug=cfg["slug"], name=cfg["name"], types=cfg.get("types", ["code"]),
            pipeline_scoring=bool(cfg.get("pipeline_scoring")),
            sdk_call_rx=c("sdk_call"), dependency_rx=c("dependency"),
            inline_pipe_rx=c("inline_pipe"), runner_rx=c("runner"), http_rx=c("http"),
            hosted_env_rx=c("hosted_env"), src_hint_rx=c("src_hint"),
    ...
    def from_ui_config(cls, name: str, cfg: dict) -> "Target":
        ...
        domains = [t.lstrip("*.").lower() for t in
                   _split_tokens(cfg.get("platform_domains"))]
        ...
        return cls(
            slug=slug, name=name, types=list(cfg.get("types") or ["code"]),
            pipeline_scoring=False,
            sdk_call_rx=_alt_regex(invoc + cli),
            dependency_rx=_alt_regex(deps),
            src_hint_rx=_alt_regex([name] + deps + ["api", "client", "server", "main",
                                                    "index", "app", "config", "lib"]),
...
def load_preset(slug: str = "rocketride") -> Target:
    return Target.from_preset(json.loads(
        (_TARGETS_DIR / f"{slug}.json").read_text(encoding="utf-8")))
