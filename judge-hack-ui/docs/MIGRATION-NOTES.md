...
- Daytona for clone + pinned Python evaluator (`tool_python` has no network). Dummy `agent_rocketride` only so Daytona / SQL `execute` can be `client.tool`; agent must not invent JSON.
...
|---|---|---|
| Secret names `ROCKETRIDE_<APP>_<PURPOSE>` | Live names are `ROCKETRIDE_DAYTONA_KEY`, `ROCKETRIDE_ANTHROPIC_KEY`, `ROCKETRIDE_GITHUB_TOKEN` | Already on the Stage 1 overlay. Renaming would break `@me` until every reference and overlay key moved together. `.env.example` documents this. |
...
| Parallel `client.use()` / `useExisting` | `clonePipelineForSandbox()` in `pool.ts` | Same `project_id` → “Pipeline is already running.” N Daytona workers need N ids. Rod’s `useExisting=true` is not a substitute. |
