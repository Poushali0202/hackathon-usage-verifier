...
import { TagPill, ThemeToggle } from './bits';
...
function DossierOverlay({ children }: { children: React.ReactNode }) {
	const { theme } = useJudgeTheme();
	const [host, setHost] = useState<Element | null>(null);
	useLayoutEffect(() => {
		setHost(document.querySelector('.jh-bleed'));
	}, []);
	const node = (
		<div className="jh jh-wide jh-dossier-host" data-theme={theme}>
			{children}
		</div>
	);
...
function ProjectDossier({ r, company, onBack }: { r: VerifyResult; company: boolean; onBack: () => void }) {
	...
	const tower = (pipelineRubric ? r.layers : r.architecture?.length) ? (
		<Tower
	...
	return (
		<>
			<div className="jh-dossier-bar">
				<button className="btn ghost sm" type="button" onClick={onBack}>← All projects</button>
				<div style={{ flex: 1, minWidth: 0 }}>
					<h2 style={{ margin: 0, fontSize: 22, letterSpacing: '-.3px' }}>{r.project || r.github}</h2>
				...
				<span className="jh-detail-meta">
					...
					{r.score != null && <span className="scorecell" style={{ fontSize: 16 }}>{Number(r.score).toFixed(1)}</span>}
					<ThemeToggle />
				</span>
			</div>
			<div className="linksrow" style={{ marginTop: 0 }}>
			...
			<div className="jh-dossier-grid">
				...
				<aside className="jh-dossier-side">
					{tower}
...
export default function ResultsGrid({ results, total, summary, exportName }: {
	...
	const selected = open != null ? results[open] : undefined;
	...
	return (
		<div className="glass" style={{ padding: '6px 0 0' }}>
			{selected && (
				<DossierOverlay>
					<ProjectDossier r={selected} company={company} onBack={() => setOpen(null)} />
				</DossierOverlay>
			)}
			...
			<div className="scrolltable">
				<table className="list runs">
					...
					<tbody>
						{results.map((r, i) => (
							<tr key={`${r.github}-${i}`}
								className={company && isFlagged(r) ? 'flagged' : undefined}
				...
				{!done && !!total && (
					<p className="muted" style={{ padding: '16px 18px 20px', fontSize: 13 }}>
						No projects in this run yet. If this batch already finished, start it again from New run.
					</p>
				)}
			</div>
		</div>
